...
import { transformationMatrix, transformPointWithMatrix , asSvgMatrix } from "./geometry.js";
...
function DocumentNode({x,y,index,name,locked,radius}) {
    ...
    const transformMatrix = transformationMatrix(x,y,1.2);
    const transform = asSvgMatrix( transformMatrix);
    const polygon = <Polygon transform={transform} vertices={vertices} className="document-path"  />

    const transformedVertices = R.map( (v) => {
        return transformPointWithMatrix(v, transformMatrix);
    }, vertices);
